import React from 'react';
import { NavLink } from 'react-router-dom';
import { Microscope, BookOpen, ClipboardList, Target, Trophy, FlaskConical } from 'lucide-react';

const navItems = [
  { name: 'Study Zone', icon: BookOpen, path: '/student/study' },
  { name: 'Virtual Lab', icon: Microscope, path: '/student/lab' },
  { name: 'Quizzes', icon: ClipboardList, path: '/student/quizzes' },
  { name: 'Missions', icon: Target, path: '/student/missions' },
  { name: 'Leaderboard', icon: Trophy, path: '/student/leaderboard' },
];

const Sidebar = ({ className = '' }) => {
  return (
    <aside className={`w-64 border-r bg-background flex flex-col h-full ${className}`}>
      {/* Logo Area */}
      <div className="h-16 flex items-center px-6 border-b border-border/50">
        <FlaskConical className="w-6 h-6 text-primary mr-2" />
        <span className="text-xl font-bold tracking-tight text-primary">ChemLearn</span>
      </div>

      {/* Navigation */}
      <nav className="flex-1 overflow-y-auto px-3 py-6 space-y-1">
        {navItems.map((item) => (
          <NavLink
            key={item.name}
            to={item.path}
            className={({ isActive }) =>
              `flex items-center px-3 py-2.5 text-sm font-medium rounded-md transition-colors ${
                isActive
                  ? 'bg-primary/10 text-primary'
                  : 'text-muted-foreground hover:bg-muted hover:text-foreground'
              }`
            }
          >
            <item.icon className="w-5 h-5 mr-3 flex-shrink-0" />
            {item.name}
          </NavLink>
        ))}
      </nav>
    </aside>
  );
};

export default Sidebar;
